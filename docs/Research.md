...
- **Bike Lock WIKI:** A security-oriented review and recommendation site for bicycles, which provides a comprehensive diagram of bike parts. The most important parts are shown below:

```{figure} ../Images/Bike_Parts.png
---
name: Bike_Parts
---
Bike parts found in bikelockwiki.com
```

- **GeometryGeeks.bike:** For geometry, the site GeometryGeeks.bike offers a clear reference image and thorough explanations of all relevant measurements.

## Prototype Reference

To create a base prototype for the bike, a temporary template was needed. This was sourced from a research paper on bicycle frame optimization (Yung-Chang Cheng and Cheng-Kang Lee and Ming-Tsang Tsai, 2016). The example image below, taken from the paper, provided reference values for the length and thickness of each bike part:
...
name: Bike_Geometry
---
Bike geometry found in geometrygeeks.bike
```

## Data Collection

A single prototype is not sufficient to ensure the model's flexibility. Therefore, several examples were gathered from geometrygeeks.bike and its database of bikes and measurements. This allowed for a broader understanding of the range of possible geometries and ensured the model could accommodate various designs.
